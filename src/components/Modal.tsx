...
export default function Modal({
  activity,
  onClose
}: Readonly<{ activity: ActivityData; onClose: () => void }>) {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center bg-[#000000c0]">
      <div className="modal-content relative min-w-[300px] w-[25vw] min-h-52 bg-inherit p-5 rounded border border-solid border-[#272727]">
        <button
          className="absolute top-1 right-3 border-0 text-inherit cursor-pointer bg-none font-extrabold text-lg"
          onClick={onClose}
        >
          x
        </button>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <p>
            <b>Data de criação:</b> {startDate}
          </p>
          <p>
            <b>Atividade ativa:</b> {isActive ? 'Sim' : 'Não'}
          </p>
